...
_Bool NET_DEVICE_Connect(char *type, char *ip, char *port)
{
	...
	if(*type == 'T')
		status = NET_DEVICE_SendCmd(cmd_buf, "CONNECT");
	else if(*type == 'U')
		status = NET_DEVICE_SendCmd(cmd_buf, "BIND OK");
...
unsigned char NET_DEVICE_Check(void)
{
	...
	while(--err_count)
	{
		...
		{
			if(strstr(net_device_info.cmd_resp, "CONNECT OK"))					//已连接
			{
