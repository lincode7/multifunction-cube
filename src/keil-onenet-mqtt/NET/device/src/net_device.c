...
#if (PHONE_AP_MODE == 1)
#define ESP8266_WIFI_INFO "AT+CWJAP=\"WOOD\",\"57261361cc91\"\r\n"
#endif
...
************************************************************
*/
__inline static void NET_DEVCIE_Delay(unsigned int time)
{
...
unsigned int NET_DEVICE_GetTime(void)
{
	...
	char *ntp_server[] = {"cn.ntp.org.cn", "edu.ntp.org.cn", "tw.ntp.org.cn"};
	...
	while (time_out--) //等待
	{
		...
		if (wifi_status || key_count) //如果收到数据
		{
			...
			else if (wait_count >= 10 || key_count)
			{
				...
				while (1)
				{
					MCU_GPIO_Output_Ctl("led1", 0);
					MCU_GPIO_Output_Ctl("led2", 0);
					MCU_GPIO_Output_Ctl("led3", 0);
					MCU_GPIO_Output_Ctl("led4", 0);
					...
					if (strstr((char *)net_device_info.cmd_resp, "Smart get wifi info"))
					{
						...
						MCU_GPIO_Output_Ctl("led1", 1);
						MCU_GPIO_Output_Ctl("led2", 1);
						MCU_GPIO_Output_Ctl("led3", 1);
						MCU_GPIO_Output_Ctl("led4", 1);
...
_Bool NET_DEVICE_Init(void)
{
	...
	case 3:
		...
		if (!NET_DEVICE_SendCmd(ESP8266_WIFI_INFO, "GOT IP"))
#endif
			net_device_info.init_step++;
		break;
